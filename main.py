...
def callback_function1(callback_obj):
    keyboard = telebot.types.InlineKeyboardMarkup()
    button71 = telebot.types.InlineKeyboardButton(text="Программирование и информационные технологии",callback_data="button71")
    button72 = telebot.types.InlineKeyboardButton(text="Медицинский", callback_data="button72")
    button73 = telebot.types.InlineKeyboardButton(text="Экономика и управление", callback_data="button73")
    button74 = telebot.types.InlineKeyboardButton(text="Строительство и энергетика", callback_data="button74")
    button75 = telebot.types.InlineKeyboardButton(text="Юриспруденция", callback_data="button75")
    button76 = telebot.types.InlineKeyboardButton(text="Реклама и СМИ", callback_data="button76")
    button77 = telebot.types.InlineKeyboardButton(text="Сервис и туризм", callback_data="button77")
    button78 = telebot.types.InlineKeyboardButton(text="Педагогика и психология", callback_data="button78")
    button79 = telebot.types.InlineKeyboardButton(text="Дизайн", callback_data="button79")
    ...
    bot.send_message(callback_obj.from_user.id, "вы выбрали ВУЗ, давайте выберем факультет", reply_markup=keyboard)
...
def callback_function2(callback_obj):
    keyboard = telebot.types.InlineKeyboardMarkup()
    button21 = telebot.types.InlineKeyboardButton(text="Программирование и информационные технологии",callback_data="button21")
...
def bally_ege(message):
    keyboard = telebot.types.InlineKeyboardMarkup()
    button2 = telebot.types.InlineKeyboardButton(text="Математика", callback_data="button2")
...
def bally_oge(message):
    ...
    jjj = bot.send_message(message.from_user.id, "Пожалуйста введите предметы, которые вы сдавали" ,  reply_markup=keyboard)
    bot.register_next_step_handler(jjj, predmety1)
...
def predmety1(message):
    ...
    bot.send_message(message.from_user.id, "Сейчас выдам список ПТУ и подходящих вам профилей\n"                                           "- [Текст ссылки](https://example.com)", parse_mode='MARKDOWN')
...
def predmety2(message):
    ...
    bot.send_message(message.from_user.id, "Сейчас выдам список ВУЗов и подходящих вам профилей\n"                     "- [Текст ссылки](https://example.com)", parse_mode='MARKDOWN')
